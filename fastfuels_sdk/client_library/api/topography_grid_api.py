...
class TopographyGridApi:
    ...
    def create_topography_grid(
        ...
        # Create Topography Grid  This endpoint creates a new topography grid resource for a specific domain. Topography grid data represents various topography attributes (e.g., elevation) that exist within the spatial context of a domain. The topography grid data can be generated using a variety of data sources (e.g., LANDFIRE, uniform values) and interpolation methods.  On resource creation, the topography grid data is set to a status of \"pending\". The topography grid data is generated in the background using the specified method and data sources. Once the topography grid data is generated and available for user access, the status is updated to \"completed\".  ## Endpoint  ``` POST /v1/domains/{domainId}/grids/topography ```  ## Path Parameters  - `domainId` (string, required): The unique identifier of the domain for   which you want to create the topography grid.  ## Request Body  The request body should be a JSON object containing the following fields:  - `attributes` (array of strings, required): List of attributes to include in the topography grid. Currently, the only possible value is:   - `\"elevation\"`  ### Attribute Descriptions and Sources  - `elevation` (m): The elevation of the terrain above sea level. - `aspect` (degrees): The aspect of the terrain surface (direction the slope faces). - `slope` (degrees): The slope of the terrain surface.  Each attribute can be sourced from different data sources. By default, the system will use pre-defined data sources to acquire data for the specified attributes. However, if you want to specify additional information for how to acquire an attribute, you can provide detailed instructions in the request body.  To specify additional information for an attribute, add a field with the attribute name to the request body. For example:  ```json {     \"attributes\": [\"elevation\"],     \"elevation\": {         \"source\": \"LANDFIRE\",         \"version\": \"2020\",         \"interpolationMethod\": \"linear\"     } } ```  The general form of this construction is as follows:  - `elevation` (object, optional): The data source for the elevation attribute. The structure of this object depends on the source specified.  #### Uniform Source  If the source is `\"uniform\"`, the object should have the following fields:  - `source` (string, required): Must be `\"uniform\"`. - `value` (float, required): The uniform value for the attribute.  #### LANDFIRE Source  If the source is `\"LANDFIRE\"`, the object should have the following fields:  - `source` (string, required): Must be `\"LANDFIRE\"`. - `version` (string, required): The version of the LANDFIRE product. Currently, only `\"2020\"` is supported. - `interpolationMethod` (string, required): The interpolation method to use. Possible values are:   - `\"nearest\"`   - `\"linear\"`   - `\"cubic\"`   - `\"zipper\"`  ## Response  If the request is successful, the endpoint will return a `201 Created` status code and the created topography grid resource in the response body. The response body will be a JSON object with the following fields:  - `attributes` (array of strings): The list of attributes included in the topography grid. - `elevation` (object): The data source information for the elevation attribute. - `status` (string): The status of the topography grid. Will be set to `\"pending\"` initially. - `createdOn` (string): The timestamp when the topography grid was created. - `modifiedOn` (string): The timestamp when the topography grid was last modified. - `checksum` (string): A unique checksum for the topography grid resource.  ## Error Responses  - `404 Not Found`: The specified domain does not exist or the user does not have access to it. - `422 Unprocessable Entity`: The request body is invalid, the data sources are not compatible with the domain CRS, or required fields are missing.
    ...
    def create_topography_grid_export(
        ...
        # Create Topography Grid Export  ## Endpoint  ``` POST /v1/domains/{domainId}/grids/topography/exports/{exportFormat} ```  ## Description  This endpoint initiates the creation of an export file for the topography grid data of a specific domain in the zarr compressed format. The export process runs in the background.  ## Path Parameters  - `domainId` (string, required): The unique identifier of the domain. - `exportFormat` (string, required): The format for the export. Currently, only `\"zarr\"` is supported.  ## Response  If successful, returns a `201 Created` status code and a JSON object with:  - `status` (string): Initially set to `\"pending\"`. - `createdOn` (string): Timestamp of export request creation. - `modifiedOn` (string): Timestamp of last modification. - `expiresOn` (string): Expiration timestamp (7 days from creation).  ## Error Responses  - `404 Not Found`: Domain not found or user lacks access. - `422 Unprocessable Entity`: Invalid export format or request. - `429 Too Many Requests`: Error submitting the export job due to resource exhaustion.  ## Usage Notes  - Ensure the topography grid for the domain is completed before initiating an export. - Check the export status using the GET endpoint.
    ...
    def get_topography_grid_attribute_metadata(
        ...
        # Get Topography Grid Attribute Metadata  Retrieves metadata about the structure of the topography grid and its attributes for a given domain.  ## Endpoint  ``` GET /v1/domains/{domainId}/grids/topography/attributes ```  ## Path Parameters  - `domainId` (string, required): Unique identifier of the domain.  ## Response  ### Success Response (200 OK)  Returns a JSON response containing:  - `shape`: Dimensions of the grid data. - `dimensions`: Names of each dimension. - `chunks`: Number of chunks in each dimension. - `chunkShape`: Shape of each chunk. - `attributes`: Detailed information about each available attribute.  ### Error Responses  - 404 Not Found: Topography grid not found or not accessible. - 422 Unprocessable Entity: Topography grid not in 'completed' status.
